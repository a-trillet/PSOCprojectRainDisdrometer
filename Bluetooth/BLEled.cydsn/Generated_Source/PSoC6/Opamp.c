...
*
* Description:
*  This file provides the source code to the API for the Opamp
*  component.
*
* Note:
*  None.
...
uint8_t Opamp_initVar = 0u;

/* Initialize opampPower to customizer selection. */
cy_en_ctb_power_t Opamp_opampPower = CY_CTB_POWER_HIGH;
...
****************************************************************************//**
*
* Initialize the Opamp with default customizer
* values when called the first time and enables the Opamp.
* For subsequent calls the configuration is left unchanged and the opamp is
* just enabled.
*
* Opamp_initVar: this global variable is used to indicate the initial
* configuration of this component. The variable is initialized to zero and set
* to 1 the first time Opamp_Start() is called. This allows
* enabling/disabling a component without re-initialization in all subsequent
* calls to the Opamp_Start() routine.
*
*******************************************************************************/
void Opamp_Start(void)
{
    if(0u == Opamp_initVar)
    {
       Opamp_Init();         /* Initialize and turn on the hardware block. */
       Opamp_initVar = 1u;
    }
